...
class myPlayer(AdvancePlayer):
    ...
    def StartRound(self,game_state):
        self.roundno += 1
        #logging.debug("round: %d ",self.roundno)
        return None
    ...
    def SelectMove(self, moves, game_state):
        ...
        for move in moves:
            if move[2].num_to_floor_line == 0:
                nxx += 1
        logging.debug("%d, %d", len(moves),nxx)'''
        ...
        def minimax(gamecopy, depth, maximizingPlayer,start_t):
            ...
            if depth == 0 or cur_t>max_turn_time or not (gamecopy.TilesRemaining()):
                ...
                weight = numpy.interp(depth_int-depth, [0, 3*depth_int], [0, 1])
        ...
        for move_me1 in moves_me1:
            ...
            if cut_num>10:
                roundscore_me = minimax(gs_copy1, depth_int-1, False, start_time)
            else:
