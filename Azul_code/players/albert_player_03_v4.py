...
# changed ranking method
#combined 3 ranking method, tried to cut move and increase depth

#not a particular effective version
...
class myPlayer(AdvancePlayer):
    ...
    def SelectMove(self, moves, game_state):
        ...
        def num_to_line_v2(column_num, moves, my_id, my_gamestate):
            ...
            # return list(zip(move_colour_num, move_tile_num, move_index))
        ...
        def num_to_line_v3(column_num, moves, my_id, my_gamestate):
            ...
            for i in moves:
                ...
                move_tile_num.append(i[2].num_to_pattern_line)

                #array for number of tiles per colour in round
                move_colour_num.append(Max_num_per_colour(i[2].tile_type))
                
                #array for num of tiles filled in the same vertical column
                move_col = int(my_gamestate.players[my_id].grid_scheme[i[2].pattern_line_dest][i[2].tile_type])
                move_vert_num.append(column_num[move_col])

                #array for index
                move_index.append(count)
                count = count + 1
            return list(zip(move_tile_num, move_colour_num, move_vert_num, move_index))
            # return list(zip(move_colour_num, move_tile_num, move_index))
        ...
        #create moves for ranking using three metrics
        def create_move_3(rank_moves, unrank_moves, cut_num):
            new_move = []
            for x in rank_moves:
                num = x[3] #read index
                new_move.append(unrank_moves[num]) #assign move element at index to new array
        ...
        def minimax(gamecopy, depth, maximizingPlayer,start_t):
            ...
            if maximizingPlayer:
                ...
                b1 = num_to_line_v3(a1, moves_me, self.id, gamecopy)
                c1 = rank_move_3(b1, cut_num)
                moves_me = create_move_3(c1, moves_me, cut_num)
                ...
                b3 = num_to_line_v3(a3, moves_op, self.opid, gamecopy)
                c3 = rank_move_3(b3, cut_num)
                moves_op = create_move_3(c3, moves_op, cut_num)
        ...
        b1 = num_to_line_v3(a1, moves, self.id, game_state)
        c1 = rank_move_3(b1, cut_num)
        # logging.debug("rank_move: %s", c1)
        moves_me1 = create_move_3(c1, moves, cut_num)
